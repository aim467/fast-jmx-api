...
import com.fast.jmx.domain.oshi.DisksInfo;
import com.fast.jmx.domain.oshi.NetworkInfo;
...
public class SystemInfoUtil {
    ...
    public static OSInfo getSystemInfo() {
        ...
        //主机信息
        InetAddress address = FastJmxUtils.getLocalHostExactAddress();
        osInfo.setHost(address.getHostAddress());
        osInfo.setHostName(address.getHostName());
        //系统启动时间
        osInfo.setBootTime(formatBootTime(getBootTime()));
        return osInfo;
    }


    private static String formatBootTime(String bootTime) {
        if (StrUtil.isNotEmpty(bootTime)) {
            DateTime start = DateUtil.parse(bootTime, DatePattern.NORM_DATETIME_PATTERN);
            return DateUtil.formatBetween(start, DateUtil.date());
        }
        return "";
    }
